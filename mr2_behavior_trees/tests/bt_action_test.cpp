...
TEST(bt_action, recharge_btn)
{
  ...
  std::string pkgpath = ament_index_cpp::get_package_share_directory("mr2_behavior_trees");
  std::string xml_file = pkgpath + "/behavior_tree_xml/recharge.xml";
...
TEST(bt_action, patrol_btn)
{
  ...
  std::string pkgpath = ament_index_cpp::get_package_share_directory("mr2_behavior_trees");
  std::string xml_file = pkgpath + "/behavior_tree_xml/patrol.xml";
  ...
  ASSERT_NEAR(node_sink->vel_msgs_.size(), 50, 2);
...
TEST(bt_action, get_waypoint_btn)
{
  ...
  node->declare_parameter("recharge");
  node->declare_parameter("wp1");
  node->declare_parameter("wp2");
  node->declare_parameter("wp3");

  std::vector<std::string> pw_ids = {"recharge", "wp1", "wp2", "wp3"};
  std::vector<double> recharge_wp = {3.67, -0.24};
  ...
  node->set_parameter({"wp2", wp2});
  node->set_parameter({"wp3", wp3});
...

  {
    ...
    std::string pkgpath = ament_index_cpp::get_package_share_directory("mr2_behavior_trees");
    std::string xml_file = pkgpath + "/behavior_tree_xml/get_waypoint.xml";
...

  {
    ...
    std::string pkgpath = ament_index_cpp::get_package_share_directory("mr2_behavior_trees");
    std::string xml_file = pkgpath + "/behavior_tree_xml/get_waypoints.xml";
...
TEST(bt_action, battery_checker_btn)
{
  ...
  std::string pkgpath = ament_index_cpp::get_package_share_directory("mr2_behavior_trees");
  std::string xml_file = pkgpath + "/behavior_tree_xml/battery_check.xml";
  ...
  float battery_level;
  ASSERT_TRUE(blackboard->get("battery_level", battery_level));
  ASSERT_NEAR(battery_level, 98.32, 0.1);
}
